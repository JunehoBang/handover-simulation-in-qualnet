...
void Layer3LteProcessEvent(Node* node, UInt32 interfaceIndex, Message* msg)
{
    ...
    switch (eventType)
    {
		...
		case MSG_RRC_LTE_Handover_Waiting_Timer_Expired:
		{
			Layer3LteHandoverWaitingTimerExpired(node, interfaceIndex,msg);
			break;
		}
        default:
        {
    ...
    Node* node, int interfaceIndex, const NodeInput* nodeInput)
{
  ...
  //      ERROR_ReportError(errorMessage);
	...
	node->lteMeasurementReportTxInterval = reportInterval;
											...
											std::list<MeasurementReport>* measurementReport)
{
	...
	//int reportSize= measurementReport->size();

	//LteRnti targetRnti = LTE_INVALID_RNTI;
	//double maxServRSRP = measurementReport->begin()
	...
	//만약 핸드오버 진행중인 UE로부터 measurement report를 받는다면 나가기
                    ...
                    const RrcConnectionReconfiguration& reconf)
{
	...
	Layer3DataLte* layer3Data =
		LteLayer2GetLayer3DataLte(node, interfaceIndex);
	ExecutingHandoverDB* executingHandoverDB = layer3Data->executingHandoverDB;
	ExecutingHandoverDB::iterator it;
	ExecutingHandover executingHandover;
	for (it=executingHandoverDB->begin();it!=executingHandoverDB->end();it++)
	{
		if(it->ueRnti == hoParticipator.ueRnti)
			break;
	}
	if(it!=executingHandoverDB->end())
	{

		executingHandover = *it;
		executingHandover.hoPreparation = node->getNodeTime();
		executingHandoverDB->erase(it);
		executingHandoverDB->insert(executingHandover);
	}
                    ...
                    const RrcConnectionReconfiguration& reconf)
{
	...
	Layer3DataLte* layer3Data=
		LteLayer2GetLayer3DataLte(node, interfaceIndex);
	layer3Data->handoverExecutionUnderway= true;
